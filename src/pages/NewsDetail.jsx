...
export default function NewsDetail({ slug }) {
  ...
  const img = useMemo(() => asset(((item?.image) || '').replace(/^\//, '')), [item]);
    ...
    <main className="pt-24 md:pt-28 pb-20">
      <Container>
        ...
        <div className="mt-6 rounded-2xl overflow-hidden ring-1 ring-black/10 bg-slate-50">
          <img src={img} alt={item.title} className="w-full h-auto" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = PLACEHOLDER_IMG; }} />
        </div>
